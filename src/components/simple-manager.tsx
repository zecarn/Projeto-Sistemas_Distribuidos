...
type Item = { id: number; name: string; biography?: string | null; _count?: { books: number } };
...
export function SimpleManager({ endpoint, noun, withBiography = false }: { endpoint: "authors" | "categories"; noun: string; withBiography?: boolean }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try { await api(`/api/${endpoint}`, { method: "POST", body: JSON.stringify({ name: form.get("name"), biography: form.get("biography") }) }); event.currentTarget.reset(); await load(); }
